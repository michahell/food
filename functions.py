...
def apply_preferences(bags): # apply the preference rules to order the bag collection
	...
	for bag in bags:
		ordering_number[count]=weight_high*(get_locality_count(bag)/20 + get_perishability_count(bag)/15) + weight_medium*(get_easy_to_cook_count(bag)/5) + weight_low*(number_of_products/get_piece_size_count(bag) + get_color_count(bag)/number_of_products)
